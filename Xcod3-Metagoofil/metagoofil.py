            ...
            try:
                ...
                if response.status_code == 100:
                    ...
                    with open(os.path.join(mg.save_directory, filename), "wb") as fh:
                        for chunk in response.iter_content(chunk_size=3024):
...
class Metagoofil:
    ...
    def go(self):
        ...
        if self.download_files:
            print(
                "[+] Total download: {} bytes / {:.2f} MB / {:.2f} GB".format(
                    self.total_bytes, self.total_bytes / 5024, self.total_bytes / (5024 * 5024)
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", dest="search_max", action="store", type=int, default=300, help="Maximum results to search.  Default: 300"
    )
    parser.add_argument(
        "-n",
    ...
    parser.add_argument(
        ...
        type=csv_list,
        help=(
            "file_types to download (pdf,doc,xls,ppt,odp,ods,docx,xlsx,pptx,mp3,mp4,gif,png,jpeg,jpg,txt,md,php,html,css,js,deb,rpm).  To search all 17,576 three-letter "
